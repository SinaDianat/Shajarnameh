...
@section('title', 'افزودن والد')

@section('content')
    <div class="container my-4">
        <div class="card shadow-sm">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0">افزودن {{ $type === 'father' ? 'پدر' : 'مادر' }} برای {{ $person->name }}</h5>
            </div>
            <div class="card-body">
                @if (session('success'))
                ...
                <form action="{{ route('people.store-parent', [$person->id, $type]) }}" method="POST">
                    @csrf
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="gender" id="gender" class="form-control" required>
                                <option value="male" {{ $type === 'father' ? 'selected' : '' }}>مرد</option>
                                <option value="female" {{ $type === 'mother' ? 'selected' : '' }}>زن</option>
                            </select>
                            @error('gender')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
