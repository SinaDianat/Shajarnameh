    ...
    <div class="container my-4">
        <div class="card shadow-sm">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0">افزودن فرد جدید</h5>
            </div>
            <div class="card-body">
                @if ($errors->any())
                ...
                <form action="{{ route('people.store') }}" method="POST">
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" class="form-check-input" id="is_deceased" name="is_deceased" {{ old('is_deceased') ? 'checked' : '' }}>
                        <label class="form-check-label" for="is_deceased">فوت شده است</label>
                    </div>
                    <div id="deceased-fields" class="mt-3" style="display: {{ old('is_deceased') ? 'block' : 'none' }};">
                        <div class="mb-3">
                            <label for="city_of_die_input" class="form-label">شهر محل فوت</label>
                            <input type="text" id="city_of_die_input" class="form-control" placeholder="جستجوی شهر..." value="{{ old('city_of_die_input') }}" autocomplete="off">
                            <input type="hidden" name="city_of_die" id="city_of_die" value="{{ old('city_of_die') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" id="date_of_die" name="date_of_die" value="{{ old('date_of_die') }}">
                            @error('date_of_die')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
